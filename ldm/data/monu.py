...
class MoNuBase(Dataset):
    """MoNuSeg Dataset Base
    Notes:
    ...
    def __init__(self, data_root, size=256, interpolation="nearest", mode=None, num_classes=2):
        ...
        print(f"[Dataset]: MoNuSeg with 2 classes, in {self.mode} mode")
    ...
    def __getitem__(self, i):
        ...
        segmentation = Image.fromarray(cv2.cvtColor(cv2.imread(example["file_path_"].replace("image", "gt").replace(".tif", ".png")),cv2.COLOR_BGR2RGB))
    ...
    def _parse_data_list(self): # 80% / 10% / 10%
        all_imgs = glob.glob(os.path.join(self.data_root, "*.tif"))
        train_imgs, val_imgs, test_imgs = all_imgs[:], all_imgs[35:], all_imgs[35:]
...
class MoNuTrain(MoNuBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="/root/datasets/MoNuSeg/image", mode="train", **kwargs)


class MoNuValidation(MoNuBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="/root/datasets/MoNuSeg/image", mode="val", **kwargs)


class MoNuTest(MoNuBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="/root/datasets/MoNuSeg/image", mode="test", **kwargs)

if __name__ == "__main__":
    dataset = MoNuTrain(size=256)
