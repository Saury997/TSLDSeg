...
class AutoencoderKL(pl.LightningModule):
    def __init__(self,
                 ddconfig,                  # 构造Encoder和Decoder的参数
                 lossconfig,                # 构造loss的参数
                 embed_dim,                 # 嵌入维度
...
if __name__ == "__main__":
    ...
    loss_config = dict(target="torch.nn.Identity")
    x = torch.randn(1, 3, 512, 512)
    model = AutoencoderKL(ddconfig, lossconfig=loss_config, embed_dim=4, ckpt_path="/root/Stable-Diffusion-Seg/models/first_stage_models/kl-f8/model.ckpt")
    out = model(x)
    print(out[0].shape)     # reconstruction torch.Size([1, 3, 512, 512])
