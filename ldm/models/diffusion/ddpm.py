...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
                 unet_config,               # U-net 的配制参数
                 timesteps=1000,            # 扩散模型的总时间步数
                 beta_schedule="linear",    # 噪声水平beta的调整策略
                 loss_type="l1",            # 计算噪声预测误差的损失函数的类型
                 ckpt_path=None,            # 加载 checkpoint 文件的路径
                 ignore_keys=[],            # 使用 checkpoint 加载模型时忽略的键列表
                 load_only_unet=True,       # 是否只加载 UNet 的权重
                 monitor="val/loss",
                 use_ema=True,              # 是否使用指数移动平均 (EMA) 来平滑模型参数
                 first_stage_key="image",   # 第一阶段模型中使用的键名
                 image_size=256,
                 channels=3,
                 log_every_t=100,           # 在生成过程中每隔多少时间步t记录一次图片
                 clip_denoised=True,        # 是否将噪音裁剪至(−1.0,1.0)区间
                 linear_start=1e-4,         # beta_0
                 linear_end=2e-2,           # beta_T
                 cosine_s=8e-3,             # 控制余弦增大的参数
                 given_betas=None,          # 直接给定一组beta
                 v_posterior=0.,  # weight for choosing posterior variance as sigma = (1-v) * beta_tilde + v * beta
                 original_elbo_weight=0.,   # 损失函数中使用原始证据下界 (ELBO) 的权重
                 l_simple_weight=1.,        # 简单损失的权重
                 conditioning_key=None,     # 使用条件生成时, 条件数据的键
                 parameterization="eps",    # 模型参数化的方式, 即 UNet 预测原始图像还是噪声
                 scheduler_config=None,     # 优化器的配置参数
                 use_positional_encodings=False,    # 是否使用位置编码
                 learn_logvar=False,        # 是否学习对数方差的参数
...
class LatentDiffusion(DDPM):
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        if self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0:  # and not self.restarted_from_ckpt:
            ...
            self.log("test_avg_dice", 0, prog_bar=False, logger=True, on_step=True, on_epoch=False)
    ...
    def get_loss_seg_regression(self, x_start, x_noisy, t, model_output, seg_loss_type='dice'):
        def dice_loss(pred, target, smooth=1e-6):
            """ 计算 Dice Loss，适用于分割任务 """
            target = torch.relu(target)
            pred = torch.relu(pred)  # 先进行 sigmoid 激活
            intersection = (pred * target).sum(dim=(2, 3))  # 计算交集
            union = pred.sum(dim=(2, 3)) + target.sum(dim=(2, 3))  # 计算并集
            dice_score = (2. * intersection + smooth) / (union + smooth)  # 计算 Dice 系数
            return 1 - dice_score.mean()  # Dice Loss = 1 - Dice 系数
        # TODO 添加针对图像分割的loss
    ...
    def p_losses(self, x_start, cond, t, seg_label=None, noise=None):
        ...
        loss_seg = self.get_loss_seg_regression(x_start, x_noisy, t, model_output, seg_loss_type='default').mean([1, 2, 3])
    ...
    def log_dice(self, data=None, save_dir=None, ddim_steps=50):
        ...
        metrics_dict.update({"test_avg_dice/direct_ema": np.mean(multi_dice)})
        metrics_dict.update({"test_avg_iou/direct_ema": np.mean(multi_iou)})
        for cls in range(1, self.num_classes):
            metrics_dict.update({f"test_avg_dice/direct_ema_{cls}": multi_dice[cls-1]})
        for cls in range(1, self.num_classes):
            metrics_dict.update({f"test_avg_iou/direct_ema_{cls}": multi_iou[cls-1]})
        ...
        metrics_dict.update({"test_avg_dice": list(multi_dice)})
        metrics_dict.update({"test_avg_iou": list(multi_iou)})
